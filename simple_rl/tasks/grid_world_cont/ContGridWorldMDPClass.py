...
class ContGridWorldMDP(MDP):
    ...
    def __init__(self,
        ...
        init_state = ContGridWorldState(init_loc[0], init_loc[1])

        MDP.__init__(self, ContGridWorldMDP.ACTIONS, self._transition_func, self._reward_func, init_state=init_state, gamma=gamma)
        ...
        self.cur_state = ContGridWorldState(init_loc[0], init_loc[1])
    ...
    def _transition_func(self, state, action):
        ...
        if action == "up" and state.y < self.height and not self.is_wall(state.x, state.y + 1):
            next_state = ContGridWorldState(state.x, state.y + 1)
            ...
            next_state = ContGridWorldState(state.x + 1, state.y)
        elif action == "left" and state.x > 1 and not self.is_wall(state.x - 1, state.y):
            next_state = ContGridWorldState(state.x - 1, state.y)
        else:
            next_state = ContGridWorldState(state.x, state.y)
...
def make_grid_world_from_file(file_name, randomize=False, num_goals=1, name=None, goal_num=None, slip_prob=0.0):
    ...
    return ContGridWorldMDP(width=num_cols, height=num_rows, init_loc=(agent_x, agent_y), goal_locs=goal_locs, lava_locs=lava_locs, walls=walls, name=name, slip_prob=slip_prob)


def main():
    grid_world = ContGridWorldMDP(5, 10, (1, 1), (6, 7))
